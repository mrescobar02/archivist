...
def list_conversations(session: Session = Depends(get_session)):
    ...
    return session.exec(
        ...
        .where(ChatConversation.created_at >= cutoff)
        .order_by(ChatConversation.created_at.desc())
    ).all()


@router.post("/conversations", response_model=ChatConversationRead, status_code=201)
def create_conversation(body: ChatConversationCreate, session: Session = Depends(get_session)):
    conv = ChatConversation(title=body.title)
    session.add(conv)
    session.commit()
...
def list_messages(conv_id: int, session: Session = Depends(get_session)):
    if not session.get(ChatConversation, conv_id):
        raise HTTPException(status_code=404, detail="Conversation not found")
    return session.exec(
        select(ChatMessage)
...
def add_message(conv_id: int, body: ChatMessageCreate, session: Session = Depends(get_session)):
    if not session.get(ChatConversation, conv_id):
        raise HTTPException(status_code=404, detail="Conversation not found")
