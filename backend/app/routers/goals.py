...
def list_goals(session: Session = Depends(get_session)):
    return session.exec(select(Goal).order_by(Goal.name)).all()
...
def delete_goal(goal_id: int, session: Session = Depends(get_session)):
    ...
    for c in session.exec(select(GoalContribution).where(GoalContribution.goal_id == goal_id)).all():
        session.delete(c)
    # Unlink any savings funds that reference this goal
    for f in session.exec(select(SavingsFund).where(SavingsFund.goal_id == goal_id)).all():
        f.goal_id = None
        session.add(f)
    session.delete(goal)
    session.commit()
...
def list_goal_contributions(goal_id: int, session: Session = Depends(get_session)):
    return session.exec(
...
def create_goal_contribution(goal_id: int, body: GoalContributionCreate, session: Session = Depends(get_session)):
    goal = session.get(Goal, goal_id)
    if not goal:
        raise HTTPException(status_code=404, detail="Goal not found")
...
def delete_goal_contribution(goal_id: int, contribution_id: int, session: Session = Depends(get_session)):
