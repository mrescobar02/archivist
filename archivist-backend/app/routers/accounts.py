...
def delete_account(account_id: int, session: Session = Depends(get_session)):
    ...
    # Check references
    refs = [
        session.exec(select(Income).where(Income.account_id == account_id)).first(),
        session.exec(select(Expense).where(Expense.account_id == account_id)).first(),
        session.exec(select(FixedExpense).where(FixedExpense.account_id == account_id)).first(),
